...
SLIC::SLIC()
{
	...
	//this is for supervoxel, each lab color channel also has a depth, see destructioin function
	// for reference
	m_lvecvec = NULL;
	m_avecvec = NULL;
	m_bvecvec = NULL;
...
/*	int sz = width*height;
		...
		for( int k = 0; k < width; k++ )
		{
			...
			for( int i = 0; i < 8; i++ )
			{
				...
				if( (x >= 0 && x < width) && (y >= 0 && y < height) )
				{
					...
					if( false == istaken[index] )//comment this to obtain internal contours
						//which means, if two neighbor pixels have different label, both of them will be
						// included in the contour. Thus the contour will be thicker.
					{
						if( labels[mainindex] != labels[index] ) np++;
					}
				}
			}
			// variable np the number of 8-neighbor pixel whoes label is different from center k,j
			if( np > 1 )
			{
				contourx[cind] = k;
				contoury[cind] = j;
	...
	for( int j = 0; j < numboundpix; j++ )
	{
		...
		//draw the black line on the contour of  slic contour
		for( int n = 0; n < 8; n++ )
		{
...
//==============================================================================
///	DetectLabEdges, calculate the square of 1st order gradient magnitude
//==============================================================================
        ...
        const vector<double>&                   edges)
{
	...
	for( int n = 0; n < numseeds; n++ )
	{
		...
		//find the largest edge magnitude in the 8-neighbor
		for( int i = 0; i < 8; i++ )
		{
		...
		//if we have to move the seed, update the kseedsx[] vector
		if(storeind != oind)
		{
    ...
    const vector<double>&       edgemag)
{
	...
	for( int y = 0; y < ystrips; y++ )
	{
		...
		for( int x = 0; x < xstrips; x++ )
		{
			...
			//since x start from 0, for the first seed, it location is 0+STEP/2+xerrorOffset
	...
	// move the seed to its neighbor which has the largest edge magnitude
	if(perturbseeds)
	{
	...
	const double&				M)
{
	...
	//the compactness parameter, see the paper
	...
	for( int j = 0; j < height; j++ )
	{
		for( int k = 0; k < width; k++ )
		{
			if( 0 > nlabels[oindex] )
			{
				...
				// i think there should be break clause. Because the adjlabel might be the same as
				// the label
				//-------------------------------------------------------
				{for( int n = 0; n < 4; n++ )
				{
				...
				int count(1);
				//start from one pixel, search the same-label neighbor by 4-neighbor manner and
				//store their xy cordinate
				//.Then increas the count to store the size of this particular cluster
...
void SLIC::DoSuperpixelSegmentation_ForGivenNumberOfSuperpixels(
	...
	int the outside, int* klabels = new int[sz]; thus klabels is an int pointer or a array with the size
	of width*height. we use this function to get the label and want to make this result propagate to
	the outside function. In this case we use the reference to a pointer int *&
	*/
	int*&						klabels,
	int&						numlabels,
