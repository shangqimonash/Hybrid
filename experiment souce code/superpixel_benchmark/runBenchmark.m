...
function runBenchmark(filename)
    ...
    for parameterset_idx=1:length(p.parameterset)
        ...
        % prepare storage?????image?????????????zeros(n,1)
        imName = zeros(n,1);
        ...
        for i=1:n
            ...
            % load algorithm result?????python???????????
            %loadPath = fullfile(p.algResSavePath, p.parameterset{parameterset_idx}.name, [(names{i}) '.png']);  
            %S = imread(loadPath);
            
            %when using python?
%             loadPath = fullfile(p.algResSavePath, p.parameterset{parameterset_idx}.name, [(names{i}) '.mat']);  
%             S=load(loadPath);
%             S=S.finalLabel;
%             S=uint16(S);
            ...
            %In python use skimage.segmentation.find_boundaries(label_img)
