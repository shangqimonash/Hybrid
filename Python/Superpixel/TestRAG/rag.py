...
def min_weight(graph, src, dst, n):
    ...
    #这里get()是python dictionary里面的一个method，那个default是当src或者dst没有找到时候
    #.get()返回的default值，这里首先把default默认设成一个dictionary，也就是当n不是src或者
    #dst的neighbor时候，就返回这个default dictionary，然后再根据后面的那个['weight'],对
    #这个dictionary来hash keyValue，如果有edge，就返回edge，如果没有，就返回
    #default dictionary中‘weight'对应的那个np.inf
...
def rag_mean_color(image, labels, connectivity=2, mode='distance',
    ...
    #是这一步构成了那个region adjacent graph，也就是对于每个pixel，我求出他的右，下，以及右下
    #还有他自己4个pixel，把这四个对应的label值构成一个array，如果这个window在seg内部，那么
    #array=[0,0,0,0],表示全部都是在内部，如果这个window在seg和seg之间，那么这个时候是，
    #array=[0,1,0,2]，这个时候，对于graph的输入函数，_add_edge_filter，我就会为graph添加
    #两个edge:0->1,0->2，用这样的方法构建RAG,之后RAG其实就构件好了

    #这个函数就是把_add_edge_filter(value,graph),这个filter在每个pixel上跑一边，
    #value就是这里的footprint 这个boolean mask对应为true的那几个pixel的label的值，
    # graph对应的就是extra_arguments里面的
    #那个graph参数，mode是对边界处理的方法
    ...
    #更新graph中node的pixel count 和total color的attribute
    for index in np.ndindex(labels.shape):
        current = labels[index]
        #pixel count也就是对应这个region的pixel的数量
        graph.node[current]['pixel count'] += 1
        #total color也就是把这个region里面的img的intensity value给加上去
        graph.node[current]['total color'] += image[index]

    #更新graph中node 的mean color这个attribute
    for n in graph:
        graph.node[n]['mean color'] = (graph.node[n]['total color'] /
                                       graph.node[n]['pixel count'])

    #上一步把每个node的property给更新好了，这个时候graph中每个node都有一个pixel count
    #property表示这个node所代表的region中pixel的数量，以及他们的total color
    #当所有node都更新之后，接下来就是计算node之间edge的weight property
    #当把所有edge也都更新之后，返回这个graph，也就是建好了
