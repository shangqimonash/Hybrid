...
//in gaussian distribution, if the x is larger than 4 sigma, the value will drop to zero
#define WIDTH 4.0
...
/* make filters
 ...
 e.g. sigma=1/3. then len=ceil(1.33)+1, which indicate the gaussian distribution will be nearly zero when the pixel is 1.33 far away from the center one. We take the ceil, that is 2. The +1 means the value for gaussion[0], the weight of the center pixel
 */
...
static image<float> *smooth(image<float> *src, float sigma) {
    ...
    //this function make a half length gaussion plus the center weight mask. mask[0], mask[1] and mask[2]
    std::vector<float> mask = make_fgauss(sigma);
    normalize(mask);
    
    //tmp is the transpose of the origin input matrix. the first convolve operation is on the row wise, and was store in the row wise
...
/* convolve image with gaussian filter 
 ...
 this smooth function is on image<uchar>
 */
...
/* compute laplacian 
 ...
 the sum of second gradient on both x and y direction
